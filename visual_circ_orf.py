    ...
    for i in range(0, len(file_list), 4):
        ...
        if eval(score) >= m6a_score:
            for j in range(len(nt_str)):
                if nt_str[j:j+2] == 'AC' and eval(score_list[j]) >= eval(max(score_list))*0.75 and eval(score_list[j+1]) >= eval(max(score_list))*0.75:
...
def make_index(circrna_gtf, tmp_file_name, final_name, tmp_file_path, final_file_path):
    ...
    han_orf = open(final_file_path + '/' + final_name + '_true.fa','r+')
...
def make_circ_pic(circrna_gtf, CIRC_folder, lenth_need, tmp_file_name, tmp_file_path, final_name, final_file_path, genome_name):
    ...
    for seq_record in SeqIO.parse(han_orf, 'fasta'):
        ...
        reads_count = seq_record.id.split('$')[1].split('|')[0]
        ...
        if lenth_need*360/circ_lenth >= 15:
            draw_circle(1350, 1400, IRES_START, IRES_END-10, 'red', 'white')
            draw_arrow_tip(IRES_END-10, 10, 1375, 'red')
        else:
            draw_circle(1350, 1400, IRES_START, IRES_END, 'red', 'white')
 
        if orf_lenth*360/circ_lenth >= 15:
            draw_circle(1250, 1300, ORF_START, ORF_END-10, 'orange', 'white')
            draw_arrow_tip(ORF_END-10, 10, 1275, 'orange')
        else:
            draw_circle(1250, 1300, ORF_START, ORF_END, 'orange', 'white')
            
             
        # draw the circRNA sequence
        draw_circle(1150, 1200, 0, 360, 'lightgrey', 'white')

        # draw the junction site
        p_i = coord(0, CENTER, 1150)
        p_o = coord(0, CENTER, 1200)
        ...
        if seq_record.id in dic_file.keys():
            if dic_file[seq_record.id] != []:
                for item in dic_file[seq_record.id]:
                    ...
                    m_i = coord(m6a_angle_site, CENTER, 1150)
                    m_o = coord(m6a_angle_site, CENTER, 1200)
                    DRAW.line((m_i, m_o), fill= 'green', width=30)

        # draw the ribo-seq reads
        for line in line_reads_list:
            
            if line[0] == circ_name and int(line[1]) <= 20:
                reads_list = line[2:]
                lenth_reads = len(reads_list)
                for it in range(1,lenth_reads,3):
                    n_small = int((it + 2)/3)
                    in_r = 980 - 40*n_small
                    out_r = 1000 - 40*n_small
                    left_a = int(reads_list[it])
                    right_a = int(reads_list[it+1])
        ...
        typenum = int(4000/len(orf_id))+50
        if typenum > 200:
            typenum = 200
        arialnum = ImageFont.truetype('LiberationSans-Regular.ttf', typenum)
        DRAW.text((500, 4300), '{}({})'.format(orf_id, strand), fill='black', font = arialnum)
        ...
        DRAW.rectangle((3600, 500, 3800, 600), fill='orange')
        DRAW.text((3900, 500), 'Orf sequence', fill='black', font = arial100)
        DRAW.rectangle((3600, 700, 3800, 800), fill='red')
        DRAW.text((3900, 700), 'IRES score : {}'.format(ires_score), fill='black', font = arial100)
        DRAW.rectangle((3600, 900, 3800, 1000), fill='green')
        DRAW.text((3900, 900), 'm6A score : {}'.format(m6a_score), fill='black', font = arial100)
        

        DRAW.text((500,2000), 'circRNA_lenth = {}'.format(circ_lenth), fill='black', font = arial100)
        DRAW.text((500,2200), 'gene_id = {}'.format(gene_id), fill='black', font = arial100)
        DRAW.text((500,2400), 'ORF_lenth = {}'.format(orf_lenth), fill='black', font = arial100)
        DRAW.text((500,2600), 'ORF start site : {}'.format(orf_start_site), fill='black', font = arial100)
        DRAW.text((500,2800), 'ORF stop site : {}'.format(orf_stop_site), fill='black', font = arial100)
        DRAW.text((500,3000), 'reads count : {}'.format(reads_count), fill='black', font = arial100)
        
        DRAW.text((3280,1400), "3'", fill='black', font = arial100)
        DRAW.text((3600,1400), "5'", fill='black', font = arial100)
...
def make_circ_pic2(circrna_gtf, lenth_need, tmp_file_name, tmp_file_path, final_name, final_file_path, genome_name, WORD_folder, raw_read, raw_circrnas):
    ...
    han_circ_raw = open(raw_circrnas)
    han_amino = open(final_file_path +'/'+ final_name + '_true_translated.fa', 'r+')
    han_circ_gtf = open(circrna_gtf,'r+')
    han_reads_cover = open(tmp_file_path + '/' + genome_name + '_read_cover_jun.txt','r+')

    read_name = str(raw_read).split('/')[-1].split('.')[0]
    raw_circfile_name = str(raw_circrnas).split('/')[-1].split('.')[0]
    align_fq_file = tmp_file_path + '/' + read_name + '_' + raw_circfile_name + 'Aligned.sortedByCoord.out.bam.align_bamtofq.fastq'
    ...
    for id1 in orf_dic.keys():
        ...
        if lenth3 <= 500:
            ...
            type_num = 5
        '''
        type_num = int(2*3.14*2200/lenth3)
        if type_num > 40:
            type_num = 40
        ...
        if id1 in dic_file.keys():
            for item in dic_file[id1]:
                m6a_start = eval(item[0])
                strm6a = item[2]
                m6a_site = ires_start_site + m6a_start - 3      # correct the site
        ...
        for line in line_reads_list:
            ...
            if line[0] == name_circ and int(line[1]) <= 20:
                reads_list = line[2:]
                lenth_reads = len(reads_list)
                for it in range(0,lenth_reads,3):

                    read_id = reads_list[it]
                    tmp_seq_read_file = os.popen('grep -A1 {} {}'.format(read_id,align_fq_file))
                    strread = tmp_seq_read_file.read().split('\n')[1]
                    n_small = int((it + 3)/3)

                    out_r = 1900 - 50*n_small
                    read_start = int(reads_list[it+1])-move_num + 1

                    for i in range(len(strread)):
                        q = coord((i+read_start)*angle, CENTER, out_r)
                        DRAW.text(q, strread[i], fill = 'orange', font=arial100)
...
def express_analysis(raw_reads, tmp_file_path, final_file_path):
    ...
    orf1_name = raw_reads[0].split('/')[-1].split('.')[0]
    orf2_name = raw_reads[1].split('/')[-1].split('.')[0]

    han_orf1 = open(final_file_path + '/' + orf1_name + '_orf_filter_result' + '_true.fa','r+')
    han_orf2 = open(final_file_path + '/' + orf2_name + '_orf_filter_result' + '_true.fa','r+')
    han1_dic = {}
    han2_dic = {}
...
def main():
    ...
    for item in raw_reads:
        ...
        circrnas_file = fileload['tmp_file_location']+'/'+genome_name+'_true.fa'
        ...
        make_circ_pic2(circrna_gtf, lenth_need, tmp_file_name, tmp_file_path, final_name, final_file_path, genome_name, WORD_folder, raw_read, raw_circrnas)
